...
module.exports = function ConfigurationManager() {
    ...
    function _loadConfiguration(configPath) {
        ...
        function setConfigValues() {
            ...
            if(self.config.repositoryCache.hg && self.config.repositoryCache.hg.enabled) {
                self.config.repoCacheOptions.hg = {
                    repoCacheRoot: getRelativeFilePath(self.config.repositoryCache.hg.cacheDirectory || './hgRepoCache'),
                    hostName: self.config.repositoryCache.hg.host,
                    publicAccessURL: self.config.repositoryCache.hg.publicAccessURL || null,
                    port: self.config.repositoryCache.hg.port || 8000,
                    protocol: self.config.repositoryCache.hg.protocol || 'hg',
                    refreshTimeout: self.config.repositoryCache.hg.refreshTimeout || 10,
                    parameters: self.config.repositoryCache.hg.parameters,
                    webdirConfig: self.config.repositoryCache.hg.webdirConfig
                };
            }

            self.config.repositoryCache.enabled = self.config.repoCacheOptions.svn || self.config.repoCacheOptions.git || self.config.repoCacheOptions.hg;
